...
RELOCATION_COUNTRIES = {
    ...
    "泰國": {"lat": 13.7563, "lon": 100.5018}, # 原泰國
    ...
    # 中東
    "土耳其": {"lat": 39.9334, "lon": 32.8597}, # 已修正為首都 安卡拉 (原伊斯坦堡)
    "阿聯酋": {"lat": 24.4539, "lon": 54.3773}, # 已修正為首都 阿布扎比 (原杜拜)
    ...
    # 美洲及大洋洲
    "美國": {"lat": 38.9072, "lon": -77.0369}, # 已修正為首都 華盛頓特區 (原紐約)
    "加拿大": {"lat": 45.4215, "lon": -75.6972}, # 已修正為首都 渥太華 (原多倫多)
    "澳洲": {"lat": -35.2809, "lon": 149.1300}, # 已修正為首都 坎培拉 (原悉尼)
    "紐西蘭": {"lat": -41.2865, "lon": 174.7762}, # 已修正為首都 威靈頓 (原奧克蘭)
    "巴西": {"lat": -15.7975, "lon": -47.8919}, # 已修正為首都 巴西利亞 (原聖保羅)
    ...
    # 非洲
    "南非": {"lat": -25.7479, "lon": 28.2293}, # 已修正為行政首都 普利托利亞 (原開普敦)
    "坦桑尼亞": {"lat": -6.1722, "lon": 35.7395}, # 已修正為首都 杜篤瑪 (原達累斯薩拉姆)
...
DB_POINTS = ['太陽', '月亮', '水星', '金星', '火星', '木星', '土星', '天王星', '海王星', '冥王星', '北交點', '上升', '中天', '莉莉絲', '婚神星']
...
DB_RANKING_RULES = {
    "感情/關係類": {
        ...
        "5. 最易有穩定關係": lambda e: sum([e.asp('土星', '金星', [60,120]) or e.asp('土星', '月亮', [60,120]), e.in_s(e.rul(7), [1,4,7,10,5,9]), e.in_h('土星', [7]) and not e.is_aff('土星')]),
        "6. 最專情": lambda e: sum([e.in_s('金星', [1,7]), e.any_asp('金星', '冥王星') and e.any_asp('金星', '土星'), e.in_s('月亮', [3,9]) and e.any_asp('月亮', '土星')]),
        "7. 最浪漫": lambda e: sum([e.in_s('金星', [11]), e.asp('金星', '海王星', [0,120]), e.any_asp(e.rul(5), '海王星')]),
        "8. 最有情慾": lambda e: sum([e.in_h('火星', [8]) or e.in_s('火星', [7]), e.asp('金星', '火星', [0,90,180]), e.any_asp('月亮', '冥王星')])
    },
    "事業/成就類": {
        ...
        "2. 最有Boss潛質": lambda e: sum([e.in_h(e.rul(10), [1,10]), e.asp('土星', '冥王星', [0,90,180]) and (e.any_asp('土星', '太陽') or e.any_asp('土星', '火星')), e.cnt_s(WEIGHT_POINTS, [0,3,6,9]) >= 4]),
        ...
        "4. 最有成功潛質": lambda e: sum([(e.asp('太陽', '土星', [120]) or e.asp(e.rul(10), '土星', [120])) and (e.asp('太陽', '冥王星', [120]) or e.asp(e.rul(10), '冥王星', [120])), e.in_h(e.rul(1), [10]) or e.in_h(e.rul(10), [1]), e.asp('北交點', '中天', [0]) or e.asp('北交點', '太陽', [0])]),
        "5. 最有官運": lambda e: sum([e.in_h('太陽', [10,11]) or e.in_h('木星', [10,11]), e.any_asp(e.rul(10), e.rul(9)) or e.any_asp(e.rul(10), e.rul(11)), e.in_h('土星', [10]) and not e.is_aff('土星')]),
        "6. 最有野心": lambda e: sum([e.asp('冥王星', '太陽', [0,90]) or e.asp('冥王星', '火星', [0,90]), (e.in_s('火星', [9]) or e.in_s('土星', [9])) and (e.in_h('火星', [1,4,7,10]) or e.in_h('土星', [1,4,7,10])), e.cnt_h(WEIGHT_POINTS, [10]) >= 3]),
        "7. 最有創富力": lambda e: sum([e.in_h('金星', [2,8]) or e.in_h('木星', [2,8]) or e.in_h('冥王星', [2,8]), e.asp(e.rul(2), e.rul(10), [0,120]) or e.asp(e.rul(2), e.rul(11), [0,120]), e.any_asp('冥王星', '金星') or e.any_asp('冥王星', '木星')]),
        "8. 最有影響力": lambda e: sum([e.asp('冥王星', '中天', [0]) or e.asp('冥王星', '上升', [0]), (e.in_h('太陽', [10]) or e.in_h('月亮', [10])) and (e.any_asp('太陽', '海王星') or e.any_asp('月亮', '冥王星')), e.any_asp(e.rul(11), '太陽') or e.any_asp(e.rul(11), '月亮')]),
        "9. 最有執行力": lambda e: sum([e.in_s('火星', [9,7]), e.asp('土星', '火星', [0,60,120]), e.cnt_s(WEIGHT_POINTS, [1,5,9]) >= 3 and e.in_h('火星', [1])]),
        "10. 最有明星命": lambda e: sum([e.asp('海王星', '中天', [0]) or e.asp('金星', '中天', [0]) or e.asp('太陽', '中天', [0]), e.in_h('月亮', [10,11]) and (e.asp('月亮', '金星', [0,60,120]) or e.asp('月亮', '海王星', [0,60,120])), e.cnt_s(WEIGHT_POINTS, [4]) >= 3 and (e.any_asp('太陽', '上升') or e.any_asp('太陽', e.rul(5)))])
    },
    "運勢/生活類": {
        "1. 最有財運": lambda e: sum([e.in_h('木星', [2]) or e.in_h('金星', [2]), e.in_h(e.rul(2), [8,10,11]) and e.asp(e.rul(2), '木星', [0,60,120]), e.in_s(e.rul(2), DIGNITIES.get(e.rul(2), {}).get('廟', []) + DIGNITIES.get(e.rul(2), {}).get('旺', [])) and (e.any_asp(e.rul(2), '太陽') or e.any_asp(e.rul(2), '月亮'))]),
        "2. 最有貴人運": lambda e: sum([e.in_h('木星', [1,11]), e.asp(e.rul(11), e.rul(1), [0,120]) or e.asp(e.rul(11), '太陽', [0,120]), e.asp('太陽', '月亮', [0,60,120]) and (e.asp('月亮', '木星', [0,60,120]) or e.asp('月亮', '金星', [0,60,120]))]),
        "3. 最有旅行運": lambda e: sum([e.in_h('木星', [3,9]), e.any_asp(e.rul(9), e.rul(1)) or e.any_asp(e.rul(9), '上升'), e.cnt_s(WEIGHT_POINTS, [2,8,3,7,11]) >= 3 and e.cnt_h(WEIGHT_POINTS, [3,6,9,12]) >= 3]),
        "4. 最有健康運": lambda e: sum([e.asp('太陽', '木星', [120]) or e.in_h('木星', [1]), e.any_asp(e.rul(6), '土星') or e.any_asp(e.rul(6), '太陽'), not e.in_h(e.rul(1), [6,8,12])]),
        "5. 最有靈性": lambda e: sum([e.in_h('海王星', [8,12]) or e.in_h('冥王星', [8,12]) or e.in_h('木星', [8,12]), e.asp('月亮', '海王星', [0,120]) or e.asp('水星', '海王星', [0,120]), e.cnt_s(WEIGHT_POINTS, [11,7]) >= 3]),
        "7. 最有福氣": lambda e: sum([e.in_h('木星', [1,4]), e.asp('太陽', '木星', [120]) and e.asp('月亮', '木星', [120]) and e.asp('太陽', '月亮', [120]), e.asp('金星', '木星', [0,60,120]) and not e.is_aff('金星') and not e.is_aff('木星')]),
        "8. 最有意外之財": lambda e: sum([e.in_h('天王星', [2,8]) and (e.asp('天王星', '木星', [0,60,120]) or e.asp('天王星', '金星', [0,60,120])), e.any_asp(e.rul(8), e.rul(11)) or e.any_asp(e.rul(8), e.rul(5)), e.asp('冥王星', '木星', [0,120]) and e.in_h('冥王星', [2,8])]),
        "9. 最有家庭緣": lambda e: sum([e.in_h('月亮', [4]) or e.in_s('月亮', [3]), e.in_h('木星', [4]) or e.in_h('金星', [4]), e.any_asp(e.rul(4), e.rul(1)) or e.any_asp(e.rul(4), '太陽')])
    },
    "性格類": {
        "2. 最有熱情": lambda e: sum([e.in_s('火星', [0,4,8]), e.asp('太陽', '火星', [0,120]), e.in_s('上升', [0,4,8]) and (e.in_h('火星', [1]) or e.in_h('太陽', [1]))]),
        "3. 最有同理心": lambda e: sum([e.in_s('月亮', [3,11]) or e.in_s('海王星', [3,11]), e.asp('月亮', '海王星', [0,120]), e.in_h('月亮', [12]) or e.in_h('金星', [12]) or e.in_h('木星', [12])]),
        "4. 最聰明": lambda e: sum([e.in_s('水星', [2,5]), e.asp('水星', '天王星', [0,60,120]), e.any_asp(e.rul(3), '水星') or e.any_asp(e.rul(9), '水星')]),
        "5. 最有創意": lambda e: sum([e.in_h('海王星', [5]) or e.in_h('天王星', [5]), e.any_asp('太陽', '天王星') or e.any_asp('太陽', '海王星') or e.any_asp('水星', '天王星') or e.any_asp('金星', '海王星'), e.any_asp(e.rul(5), e.rul(1))]),
        "6. 最有耐性": lambda e: sum([e.in_s('土星', [1,5,9]), e.asp('太陽', '土星', [60,120]) or e.asp('月亮', '土星', [60,120]) or e.asp('水星', '土星', [60,120]), e.cnt_s(WEIGHT_POINTS, [1,4,7,10]) >= 3]),
        "7. 最冒險": lambda e: sum([e.in_h('木星', [1,9]) or e.in_s('木星', [8]), e.asp('火星', '天王星', [0,120]) or e.asp('火星', '木星', [0,120]), e.cnt_s(WEIGHT_POINTS, [2,5,8,11]) >= 3]),
        "8. 最有野心": lambda e: sum([e.asp('冥王星', '太陽', [0,90]) or e.asp('冥王星', '火星', [0,90]), e.in_s('土星', [9]) or e.in_s('火星', [9]), e.cnt_h(WEIGHT_POINTS, [10]) >= 3]),
        "9. 最敏感": lambda e: sum([e.in_s('月亮', [3,7,11]), e.asp('月亮', '冥王星', [0,90]) or e.asp('月亮', '海王星', [0,90]), e.in_s('上升', [3,7,11]) and e.in_h('月亮', [1])]),
        "10. 最有正能量": lambda e: sum([e.in_s('太陽', [4,8]) or e.in_s('月亮', [4,8]) or e.in_s('上升', [4,8]), e.asp('木星', '太陽', [0,120]) or e.asp('木星', '月亮', [0,120]), e.in_h('木星', [1]) and not e.is_aff('木星')]),
        "11. 最有藝術細胞": lambda e: sum([e.in_s('金星', [1,6,11]), e.asp('金星', '海王星', [0,120]), e.any_asp(e.rul(5), '金星') or e.any_asp(e.rul(12), '金星')])
    },
    "人格類": {
        "1. 最有領袖氣質": lambda e: sum([e.in_h('太陽', [1,10]), e.in_s('上升', [0,4]), e.asp('太陽', '冥王星', [0,120])]),
        "2. 最有控制慾": lambda e: sum([e.in_h('冥王星', [1,7,10]), e.asp('冥王星', '太陽', [0,90]) or e.asp('冥王星', '月亮', [0,90]) or e.asp('冥王星', '金星', [0,90]), e.cnt_s(WEIGHT_POINTS, [7]) >= 3]),
        "3. 最有叛逆精神": lambda e: sum([e.asp('天王星', '上升', [0]) or e.in_h('天王星', [1]), e.asp('天王星', '太陽', [0,90,180]) or e.asp('天王星', '月亮', [0,90,180]), e.cnt_s(WEIGHT_POINTS, [10]) >= 3]),
        "4. 最有完美主義": lambda e: sum([e.cnt_s(WEIGHT_POINTS, [5]) >= 3, e.asp('土星', '水星', [0,90]), e.any_asp(e.rul(6), '土星')]),
        "5. 最易emo": lambda e: sum([e.in_h('月亮', [8,12]), e.asp('月亮', '土星', [0,90]) or e.asp('月亮', '海王星', [0,90]), e.cnt_s(WEIGHT_POINTS, [3,7,11]) >= 3]),
        "6. 最有嫉妒心": lambda e: sum([e.in_s('月亮', [7]), e.asp('金星', '冥王星', [90,180]), e.in_h('月亮', [8]) or e.in_h('金星', [8])]),
        "7. 最自私": lambda e: sum([e.in_h('太陽', [1]) or e.in_h('火星', [1]), e.cnt_s(WEIGHT_POINTS, [0]) >= 3, e.asp('太陽', '土星', [0])]),
        "8. 最賤": lambda e: sum([e.asp('水星', '火星', [90,180]), e.in_s('火星', [2,8]) and e.is_aff('火星'), e.any_asp('天王星', '水星')]),
        "9. 最渣": lambda e: sum([e.in_s('金星', [2,8,11]) and e.any_asp('金星', '天王星'), e.any_asp(e.rul(5), '天王星') or e.any_asp(e.rul(7), '天王星'), e.in_h('火星', [12]) or e.in_h('金星', [12])]),
        "10. 最無人性": lambda e: sum([e.in_s('月亮', [1,5,9,2,6,10]) and e.in_s('水星', [1,5,9,2,6,10]), e.asp('土星', '月亮', [0]) and e.in_s('月亮', [9]), e.asp('水星', '冥王星', [90,180]) or e.asp('水星', '土星', [90,180])]),
        "11. 最有毒舌": lambda e: sum([e.in_s('水星', [5,7]), e.in_h('火星', [3]) or e.in_h('冥王星', [3]), e.any_asp('水星', '土星')]),
        "12. 最冷血": lambda e: sum([e.in_s('月亮', [9,10]), e.cnt_s(WEIGHT_POINTS, [3,7,11,0,4,8]) == 0, e.asp('金星', '土星', [90])]),
        "13. 最有報復心": lambda e: sum([e.in_s('火星', [7]) or e.in_s('冥王星', [7]), e.asp('月亮', '冥王星', [0,90]), e.any_asp(e.rul(8), e.rul(1))]),
        "14. 最有被害妄想": lambda e: sum([e.asp('海王星', '月亮', [90,180]) or e.asp('海王星', '水星', [90,180]), e.in_h('冥王星', [12]), e.any_asp('土星', '海王星')]),
        "15. 最愛操控": lambda e: sum([e.in_h('冥王星', [8,12]), e.any_asp(e.rul(10), e.rul(8)), e.cnt_s(WEIGHT_POINTS, [7,11]) >= 3]),
        "16. 最有玻璃心": lambda e: sum([e.in_s('月亮', [3,11]), e.cnt_s(WEIGHT_POINTS, [3]) >= 3, e.any_asp('上升', '月亮')]),
        "17. 最有雙面性格": lambda e: sum([e.cnt_s(WEIGHT_POINTS, [2,11]) >= 3, e.any_asp('水星', '上升'), e.any_asp(e.rul(1), e.rul(12))]),
        "18. 最有潔癖": lambda e: sum([e.in_s('上升', [5]) or e.in_s('太陽', [5]) or e.in_s('月亮', [5]), e.in_s('金星', [5]) and e.in_h('金星', [1]), e.in_s('水星', [5])]),
        "19. 最有強迫傾向": lambda e: sum([e.asp('土星', '冥王星', [0,90]), e.cnt_s(WEIGHT_POINTS, [1,4,7,10,5]) >= 4, e.in_h('土星', [6,8]) and e.in_h('冥王星', [6,8])])
    },
    "18禁類": {
        "1. 最有情慾": lambda e: sum([e.in_h('火星', [8]) or e.in_s('火星', [7]), e.asp('金星', '火星', [0,135]), e.asp('月亮', '冥王星', [0,90])]),
        "2. 最有床上魅力": lambda e: sum([e.in_h('冥王星', [5,8]), e.any_asp('金星', '冥王星') or e.any_asp('火星', '冥王星'), e.asp('莉莉絲', '火星', [0]) or e.asp('莉莉絲', '上升', [0])]),
        "3. 最持久": lambda e: sum([e.in_s('火星', [1,7,9]), e.asp('火星', '土星', [0,60,120]), e.in_s(e.rul(8), [1,4,7,10])]),
        "4. 最會挑逗": lambda e: sum([e.asp('水星', '金星', [0,120]) or e.asp('水星', '火星', [0,120]), e.cnt_s(WEIGHT_POINTS, [2,4,6]) >= 3, e.any_asp(e.rul(5), '水星')]),
        "5. 最有征服慾": lambda e: sum([e.in_s('火星', [0,9]) and e.in_h('火星', [1,10]), e.asp('火星', '冥王星', [0,90]), e.asp('太陽', '火星', [0])]),
        "6. 最敏感體質": lambda e: sum([e.in_h('月亮', [8]) or e.in_h('海王星', [8]), e.asp('金星', '海王星', [0,90]) or e.asp('火星', '海王星', [0,90]), e.cnt_s(WEIGHT_POINTS, [3,7,11]) >= 4]),
        "7. 最有性感": lambda e: sum([e.asp('冥王星', '上升', [0]) or e.in_h('冥王星', [1]), e.asp('金星', '冥王星', [0,90]), e.in_h('莉莉絲', [1]) or e.any_asp('莉莉絲', '太陽') or e.any_asp('莉莉絲', '金星')]),
        "8. 最會享受": lambda e: sum([e.in_s('金星', [1,11]) and e.in_h('金星', [2,5]), e.asp('月亮', '金星', [0,120]), e.any_asp('木星', '金星')]),
        "9. 最有禁忌吸引力": lambda e: sum([e.in_h('冥王星', [5,8]) or e.in_h('土星', [5,8]), e.in_h('金星', [12]) or e.in_h('火星', [12]), e.asp('莉莉絲', e.rul(8), [0]) or e.asp('莉莉絲', e.rul(7), [0])]),
        "10. 最有激情": lambda e: sum([e.asp('火星', '太陽', [0,120]) or e.asp('火星', '木星', [0,120]), e.in_s('金星', [0,4,8]) and e.in_s('火星', [0,4,8]), e.cnt_h(['太陽','火星'], [5,8]) >= 2]),
        "11. 最鐘意約炮": lambda e: sum([e.asp('火星', '天王星', [0,90]), e.in_s('金星', [2,8]) and e.any_asp(e.rul(5), '天王星'), e.any_asp(e.rul(8), e.rul(3))]),
        "12. 最多性伴侶": lambda e: sum([e.in_h('木星', [5,8]), e.in_s('金星', [2,5,8,11]) and e.in_s('火星', [2,5,8,11]), e.cnt_h(WEIGHT_POINTS, [5]) >= 3])
    }
}
...
chk_db_ranking = st.sidebar.checkbox("資料庫排名 (Top 3)") # 💡 新增資料庫排名
...
if st.session_state.calc_triggered:
    try:
        with st.spinner('天文運算與分析報告生成中...'):
            ...
            # 💡 [新增] 執行資料庫排名引擎
            db_rankings = {}
            if chk_db_ranking and 'imported_profiles' in st.session_state and st.session_state['imported_profiles']:
                # 遍歷資料庫計算所有人的星盤
                eval_results = []
                for p in st.session_state['imported_profiles']:
                    try:
                        p_name = p.get('Name', 'Unknown')
                        dt_p = datetime.datetime.strptime(p['born on'], "%Y-%m-%d %H:%M:%S")
                        place_str = p.get('Place', 'Hong Kong')
                        city_p = place_str.split()[-1] if ' ' in place_str else place_str
                        
                        db_jd, db_lat, db_lon, _, _ = resolve_location_and_time(city_p, dt_p.year, dt_p.month, dt_p.day, dt_p.hour, dt_p.minute)
                        db_pos, _, db_asc, db_cusps, _, db_speed = calculate_chart_engine(db_jd, db_lat, db_lon, h_code)
                        
                        evaluator = DB_Evaluator(db_pos, db_cusps, h_code, db_asc, db_speed)
                        
                        p_scores = {}
                        for category, traits in DB_RANKING_RULES.items():
                            for t_name, t_func in traits.items():
                                score = t_func(evaluator)
                                p_scores[f"{category}_{t_name}"] = score
                                
                        eval_results.append((p_name, p_scores))
                    except Exception:
                        pass
                
                # 整理排名 Top 3
                for category, traits in DB_RANKING_RULES.items():
                    db_rankings[category] = {}
                    for t_name in traits.keys():
                        key_name = f"{category}_{t_name}"
                        # 過濾有得分者，排序取前 3
                        sorted_p = sorted([res for res in eval_results if res[1].get(key_name, 0) > 0], key=lambda x: x[1][key_name], reverse=True)[:3]
                        db_rankings[category][t_name] = sorted_p
            ...
            with col_main1:
                ...
                with tabs[3]:
                    if chk_batch:
                        ...
                        st.caption("已統一轉換為標準 5 級評分制：大吉(2), 吉(1), 平(0), 凶(-1), 大凶(-2)。")
                ...
                with tabs[5]:
                    # 💡 [新增] 渲染資料庫排名 UI
                    if chk_db_ranking and db_rankings:
                        st.markdown("### 🏆 資料庫人物特質排名 (Top 3)")
                        st.caption("根據選定占星規則，對載入的 JSON 資料庫進行全自動掃描評分。")
                        
                        for cat, traits in db_rankings.items():
                            with st.expander(f"📁 {cat}", expanded=False):
                                for t_name, tops in traits.items():
                                    st.markdown(f"**{t_name}**")
                                    if not tops:
                                        st.write("- 尚無符合條件者")
                                    else:
                                        for i, (name_val, score_dict) in enumerate(tops):
                                            pts = score_dict[f"{cat}_{t_name}"]
                                            st.write(f"{i+1}. {name_val} ({pts} 條件符合)")
                                    st.divider()
                    else:
